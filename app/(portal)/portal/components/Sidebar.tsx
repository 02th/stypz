...
export default function Sidebar({ open, setOpen }: SidebarProps) {
  ...
  return (
    <>
      ...
      >
        {/* Header */}
        <div className="px-6 flex items-center justify-between h-16 border-b border-slate-100/60 shrink-0">
        ...
        {/* Nav */}
        <nav className="flex-1 flex flex-col px-4 gap-1.5 py-4 overflow-y-auto">
          ...
          {links.map((link) => {
            ...
            return (
              ...
              >
                <link.icon
                  size={18}
                  className={`transition-colors shrink-0 ${actuallyActive ? "text-blue-600" : "text-slate-400 group-hover:text-slate-600"}`}
                />
                {link.name}
              </Link>
            );
